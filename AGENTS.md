...
- **setup_caveman_skill.sh** — Installs the upstream "caveman" skill (ultra-compressed `/caveman` response mode) for both Claude and ChatGPT. Downloads from `github.com/JuliusBrussee/caveman` pinned to `CAVEMAN_REF`, then installs to each target present on the machine: Claude Code (`~/.claude/skills/caveman/SKILL.md`), Codex CLI (`~/.codex/prompts/caveman.md`), and a paste-ready Custom Instructions file for the ChatGPT web app (`~/.local/share/caveman/chatgpt-custom-instructions.md`, since the web app has no local config). Skips agents that aren't installed, backs up existing files to `.bak`. Supports `--dry-run`. Run manually.
